...
class DataStoragePage extends ConsumerWidget {
          ...
          cacheStats.when(
            data: (stats) => Column(
              children: [
                ...
                _infoTile(context, ref.L('Conversations'),
  ...
  Widget _actionTile(BuildContext context,
    ...
    return ListTile(
      leading: Icon(icon, color: EbiColors.secondaryCyan),
      title: Text(title, style: EbiTextStyles.bodyLarge),
    ...
    if (confirmed == true) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text(ref.L('Success'))));
      }
    }
  }
  ...
  Future<void> _clearAllCache(BuildContext context, WidgetRef ref) async {
    ...
    if (confirmed == true) {
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text('${ref.L('Error')}: $e')));
        }
      }
    }
  }
